...
type CalcResult = {
  ...
  methodeCout?:   string; // Explication de la méthode utilisée
};
...
function seuilDefaut(produit: any): number {
  // 1. Seuil explicitement défini sur le produit → toujours prioritaire
  if (produit.margeMin != null && produit.margeMin > 0) return produit.margeMin;
  ...
  if (produit.prixAchat > 0 && !produit.recette) return 25; // Produit revendu
  if (produit.estSemiFini) return 50;                        // Semi-fini
  return 60; // FIX 4 : boulangerie/pâtisserie → 60% par défaut (plus réaliste)
}

// ─── Calcul coût de revient ───────────────────────────────────────────────────

export function calculerCoutRevient(produit: any, beurreMP?: any): CalcResult {
  ...
  const isViennoiserie =
    recette.estViennoiserie === true ||
  ...
  let coutMP1unite = 0;
  const ingredientsAvecCout = recette.ingredients.map((ing: any) => {
  ...
  let coutRevient: number;
  let methodeCout: string;
  ...
  if (isViennoiserie) {
    ...
    if (prixBeurreReel > 0) {
      const beurreDejaInRecette = recette.ingredients.some((ing: any) =>
        ing.mp?.nom?.toLowerCase().includes("beurre") ||
  ...
  const coutTotalDetail = detailFinal.reduce((s: number, x: any) => s + x.cout, 0);
  const detailIngredients = detailFinal.map(({ ing, cout, nonGere }: any) => ({
    nom:          ing.mp?.nom ?? "—",
...
function prixConseille(coutRevient: number, margeMin: number): number | null {
  if (coutRevient <= 0 || margeMin <= 0 || margeMin >= 100) return null;
...
export async function getMargesTousProduits(companyId: string) {
  ...
  const beurreMP = await prisma.matierePremiere.findFirst({
  ...
  const marges = produits.map((produit: any) => {
    ...
    const { coutRevient, coutMP1unite, piecesParUniteRef,
            refNom, refUnite, detailIngredients, aGrammage, source, methodeCout } = calc;
...
export async function getMargesParCategorie(companyId: string) {
  const beurreMP = await prisma.matierePremiere.findFirst({
    where:   { companyId, nom: { contains: "beurre", mode: "insensitive" }, actif: true },
  ...
  return categories
    .filter((cat: any) => cat.produits.length > 0)
    .map((cat: any) => {
      const margeMin       = cat.margeMin ?? 60;
      const margesAvecPrix = cat.produits
        .filter((p: any) => (p.grammage ?? 0) > 0)
        .map((p: any) => {
          const calc = calculerCoutRevient(p, beurreMP);
          if (!calc.aGrammage || calc.source === "AUCUN") return null;
      ...
      const margeMoyenne = margesAvecPrix.length > 0
        ? margesAvecPrix.reduce((s: number, m: number) => s + m, 0) / margesAvecPrix.length
        : 0;
...
export async function simulerImpactHausse(companyId: string, variations: {
  mpId: string; variation: number;
}[]) {
  if (variations.length === 0) return { produits: [] };
  ...
  return {
    produits: produits.map((p: any) => {
      ...
      const pModifie = {
        ...
        recette: {
          ...p.recette,
          ingredients: p.recette.ingredients.map((ing: any) => ({
            ...ing,
      ...
      const beurreModifie = beurreMP && nouveauxPrix.has((beurreMP as any).id)
        ? { ...beurreMP, prixAchat: nouveauxPrix.get((beurreMP as any).id)!.nouveauPrix }
        : beurreMP;
      ...
      return {
        ...
        impacteParMPs: p.recette?.ingredients
          .filter((ing: any) => nouveauxPrix.has(ing.mp.id))
          .map((ing: any) => nouveauxPrix.get(ing.mp.id)!.nom) ?? [],
      };
    }),
